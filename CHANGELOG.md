...
- `approval.decided` events are now enriched from the approval record before rendering (the same path `approval.created` already used, renamed `approvalCreatedEventFromApi` -> `approvalEventFromApi`), so the resolved card shows the real status and decision note rather than depending on what the raw event payload carries.
